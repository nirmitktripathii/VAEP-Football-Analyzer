...
def data_generation(home_team_name, away_team_name, league_name, source):
    if source == "Latest StatsBomb (Open Data)":
        ...
        if len(match) == 0: return None, None, None
    ...
    if league_name == "Bundes Liga": league_file_name = "Bundesliga"
    elif league_name == "World Cup": league_file_name = "WorldCup-2018"
    else: league_file_name = league_name
    spadl_h5 = os.path.join('spadl', f"spadl-{league_file_name}.h5")
    if not os.path.exists(spadl_h5): return None, None, None
    with pd.HDFStore(spadl_h5) as spadlstore:
        ...
        if len(game_id) == 0: return None, None, games
        actions_list = []
        for g_id in game_id:
            action = spadlstore[f"actions/game_{g_id}"]
            action = (action.merge(spadlstore["actiontypes"], how="left")
            ...
            if actions is None:
                ...
                if len(goal) == 0:
                    ...
                    for i in range(len(goal)):
                        ...
                        if "nice_time" not in a.columns: a["nice_time"] = a.apply(nice_time, axis=1)
                        if "short_name" not in a.columns and "player_name" in a.columns: a["short_name"] = a["player_name"]
                ...
                else: st.info("Match-wise rankings are being optimized for legacy data.")
        ...
        if st.button(f"Analyze {home_team} Season Contribution"):
            ...
            with st.spinner(f"Fetching season data for {home_team}..."):
                ...
                if all_actions:
                    ...
                    st.dataframe(summary, width="stretch")
                else: st.warning("Could not find enough data for this team.")
    else: st.info("Team analytics are available for StatsBomb Open Data.")
                    ...
                    if summary is not None:
                        ...
                        if llm_reporter:
                            with st.spinner("Generating tactical intelligence report via LLM..."):
                                ...
                                st.markdown(report)
                        else: st.warning("LLM Reporter not initialized. Please set GROQ_API_KEY in Hugging Face Secrets.")
                        st.divider()
                        st.info("Detailed event-wise tactical data has been generated for advanced modeling.")
                    else: st.warning("360 data is not available for this specific match.")
            else: st.error("Match not found.")
    else: st.warning("StatsBomb 360 data is only available for 'Latest StatsBomb (Open Data)' source.")
    ...
    if data_source == "Latest StatsBomb (Open Data)":
        ...
        if st.button("Generate Season Rankings"):
            ...
            with st.spinner(f"Computing impact for {sample_size} matches..."):
                ...
                else: st.error("Failed to generate leaderboard.")
    else:
        league_file_map = {"Serie A": "VAEP_score_Serie_A.csv", "La Liga": "VAEP_score_La_Liga.csv", "Premier League": "VAEP_score_Premier_League.csv", "Ligue 1": "VAEP_score_Ligue_1.csv", "Bundes Liga": "VAEP_score_Bundesliga.csv"}
        if chosen_league in league_file_map:
            ...
            else: st.error("VAEP leaderboard not found.")
        else: st.info("Rankings available for legacy European leagues.")
